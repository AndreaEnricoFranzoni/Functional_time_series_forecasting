...
{
  vec_data = c(res_PPC_gau_no_bound$Prediction,
               res_PPC_id_no_bound$Prediction,
               res_PPC_sps_no_bound$Prediction,
               res_PPC_spt_no_bound$Prediction)
}

tot_run          = length(vec_data)
number_pred_sing = length(vec_data[[1]]$`One-step ahead prediction`)
...
barchart_alpha = ggplot(alpha_count, aes(x = reorder(Alpha,Alpha_num), y = Count)) +
  ...
  labs(title = "PPC predictions, no bounded operator", x = "Regularization parameter", y = "Times alpha being used") +
  theme_minimal()
print(barchart_alpha)

if(save_res){
  title = "count_alpha_PPC"
...
barchart_k = ggplot(k_count, aes(x = Number_PPCs, y = Count)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "PPC predictions, no bounded operator", x = "Number PPCs retained", y = "Time k beign retained") +
  theme_minimal()
print(barchart_k)
...
bp_exp_pow = ggplot(dati, aes(y = exp_pow)) +
  ...
  labs(title = "PPC predictions, no bounded operator", y = "Explanatory power retained") +
  theme_minimal() + 
