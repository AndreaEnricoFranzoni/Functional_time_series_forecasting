...
dir_stor_res = "/Test_PPC_robustness/test_non_boundness/results/prediction/"
...
source(paste0(dir_w,"/Test_PPC_robustness/test_non_boundness/utils/far_1_1d.R"))         #load functions to generate the FAR(1) process
source(paste0(dir_w,"/Test_PPC_robustness/test_non_boundness/utils/data_param.R"))       #load parameter to generate data according to a strategy
...
string_message = "
                  PPC prediction data Gaussian Kernel, norm 1.025 "
for (b in 1:N) {
...
string_message = "
                  EX prediction data Gaussian Kernel, norm 1.025 "
for (b in 1:N) {
  
  train_set = X.sample[,burnin+b-1]
  valid_set = X.sample[,burnin+b] 
...
# ----- data generation idenity Kernel norm 1.025 -----
{
  #feats of data
...
string_message = "
                  PPC prediction data identity Kernel, norm 1.025 "
for (b in 1:N) {
...
string_message = "
                  EX prediction data identity Kernel, norm 1.025 "
for (b in 1:N) {
...
string_message = "
                  PPC prediction data sloping plane t Kernel, norm 1.178 "
for (b in 1:N) {
...
string_message = "
                  EX prediction data sloping plane t Kernel, norm 1.178 "
for (b in 1:N) {
...
string_message = "
                  PPC prediction data sloping plane s Kernel, norm 1.178 "
for (b in 1:N) {
...
string_message = "
                  EX prediction data sloping plane s Kernel, norm 1.178 "
for (b in 1:N) {
