...
dir_res = paste0(dir_w,"/Test_PPC_robustness/test_non_far1/results")
...
{
  vec_data = c(res_PPC_no_far1$Prediction)
}
...
barchart_alpha = ggplot(alpha_count, aes(x = reorder(Alpha,Alpha_num), y = Count)) +
  ...
  labs(title = "PPC predictions, VAR(2) process", x = "Regularization parameter", y = "Times alpha being used") +
  theme_minimal()
...
plt_alpha_dyn=ggplot(df, aes(x = x, y = y_transformed)) +
  ...
  labs(
    title = "Regularization parameter dynamic, VAR(2)",
    x = "Time instant",
...
barchart_k = ggplot(k_count, aes(x = Number_PPCs, y = Count)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "PPC predictions, VAR(2) process", x = "Number PPCs retained", y = "Time k beign retained") +
  theme_minimal()
print(barchart_k)
...
bp_exp_pow = ggplot(dati, aes(y = exp_pow)) +
  ...
  labs(title = "PPC predictions, VAR(2) process", y = "Explanatory power retained") +
  theme_minimal() + 
