...
eps_4 <- function(t.grid)
{
  e = rcauchy(length(t.grid),0,0.0004)
  
  return(e)
}


eps_5 <- function(t.grid)
{
  e = rcauchy(length(t.grid),0,0.001)
...
noise <- function(noise_id)
{
  ...
  if(noise_id=="4") return(eps_4)                   #non wn in order to break PPC assumptions
  if(noise_id=="5") return(eps_5)                   #non wn in order to break PPC assumptions
}




far_1_1D <- function(kernel_id, noise_id, n, t.grid, a, burnin)
{
