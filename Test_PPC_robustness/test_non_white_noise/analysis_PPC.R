...
barchart_alpha = ggplot(alpha_count, aes(x = reorder(Alpha,Alpha_num), y = Count)) +
  ...
  labs(title = "PPC predictions, no white noise", x = "Regularization parameter", y = "Times alpha being used") +
  theme_minimal()
...
barchart_k = ggplot(k_count, aes(x = Number_PPCs, y = Count)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "PPC predictions, no white noise", x = "Number PPCs retained", y = "Time k beign retained") +
  theme_minimal()
print(barchart_k)
...
bp_exp_pow = ggplot(dati, aes(y = exp_pow)) +
  ...
  labs(title = "PPC predictions, no white noise", y = "Explanatory power retained") +
  theme_minimal() + 
