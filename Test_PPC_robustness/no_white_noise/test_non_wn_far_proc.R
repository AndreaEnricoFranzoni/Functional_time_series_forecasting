...
dir_stor_res = "/Test_PPC_robustness/no_white_noise/results/predictions/train_all_story"
dir_stor_res_mw = "/Test_PPC_robustness/no_white_noise/results/predictions/train_moving_window"
...
#file for saving results
{
  ...
  file_saving_PPC_gau_no_wn_mw = "/prediction_PPC_gau_no_wn_mw.Rdata"
  file_saving_PPC_id_no_wn_mw  = "/prediction_PPC_id_no_wn_mw.Rdata"
  file_saving_PPC_spt_no_wn_mw = "/prediction_PPC_spt_no_wn_mw.Rdata"
  file_saving_PPC_sps_no_wn_mw = "/prediction_PPC_sps_no_wn_mw.Rdata"
  file_saving_EX_gau_no_wn_mw  = "/prediction_EX_gau_no_wn_mw.Rdata"
  file_saving_EX_id_no_wn_mw   = "/prediction_EX_id_no_wn_mw.Rdata"
  file_saving_EX_spt_no_wn_mw  = "/prediction_EX_spt_no_wn_mw.Rdata"
  file_saving_EX_sps_no_wn_mw  = "/prediction_EX_sps_no_wn_mw.Rdata"
}

#uploading functions for generating data
source(paste0(dir_w,"/Test_PPC_robustness/no_white_noise/utils/far_1_1d.R"))         #load functions to generate the FAR(1) process
source(paste0(dir_w,"/Test_PPC_robustness/no_white_noise/utils/data_param.R"))       #load parameter to generate data according to a strategy
...
#storing prediction
{
  ...
  prediction_PPC_gau_no_wn_mw <- lapply((1):N-1,function(x) NULL)
  prediction_EX_gau_no_wn_mw  <- lapply((1):N-1,function(x) NULL)
  prediction_PPC_id_no_wn_mw  <- lapply((1):N-1,function(x) NULL)
  prediction_EX_id_no_wn_mw   <- lapply((1):N-1,function(x) NULL)
  prediction_PPC_spt_no_wn_mw <- lapply((1):N-1,function(x) NULL)
  prediction_EX_spt_no_wn_mw  <- lapply((1):N-1,function(x) NULL)
  prediction_PPC_sps_no_wn_mw <- lapply((1):N-1,function(x) NULL)
  prediction_EX_sps_no_wn_mw  <- lapply((1):N-1,function(x) NULL)
}
...
#storing errors
{
  ...
  err_PPC_gau_no_wn_en_mw <- numeric(N)     
  err_PPC_gau_no_wn_rn_mw <- numeric(N)
  err_PPC_id_no_wn_en_mw  <- numeric(N)     
  err_PPC_id_no_wn_rn_mw  <- numeric(N)
  err_PPC_spt_no_wn_en_mw <- numeric(N)     
  err_PPC_spt_no_wn_rn_mw <- numeric(N)
  err_PPC_sps_no_wn_en_mw <- numeric(N)     
  err_PPC_sps_no_wn_rn_mw <- numeric(N)
  err_EX_gau_no_wn_en_mw  <- numeric(N)     
  err_EX_gau_no_wn_rn_mw  <- numeric(N)
  err_EX_id_no_wn_en_mw   <- numeric(N)     
  err_EX_id_no_wn_rn_mw   <- numeric(N)
  err_EX_spt_no_wn_en_mw  <- numeric(N)     
  err_EX_spt_no_wn_rn_mw  <- numeric(N)
  err_EX_sps_no_wn_en_mw  <- numeric(N)     
  err_EX_sps_no_wn_rn_mw  <- numeric(N)
}
...
{
  C = 0.001
  alpha_pert = rcauchy(n=1,location = 0,scale = 1)
  #feats of data
  ...
  X.sample_gau <- far_1_1D_no_wn(kernel_id = id_kernel, noise_id = id_noise, n = n, t.grid = t.grid, a = a, burnin = burnin, pert_smooth=C, pert_add=alpha_pert)
}
...
# ----- data generation idenity Kernel norm 0.5 -----
{
  ...
  ##Simulate a stationary FAR(1) process according to a specific kernel
  X.sample_id <- far_1_1D_no_wn(kernel_id = id_kernel, noise_id = id_noise, n = n, t.grid = t.grid, a = a, burnin = burnin, pert_smooth=C, pert_add=alpha_pert)
}
...
# ----- data generation slopint plane t Kernel norm 0.5 -----
{
  ...
  ##Simulate a stationary FAR(1) process according to a specific kernel
  X.sample_spt <- far_1_1D_no_wn(kernel_id = id_kernel, noise_id = id_noise, n = n, t.grid = t.grid, a = a, burnin = burnin, pert_smooth=C, pert_add=alpha_pert)
}
...
# ----- data generation slopint plane s Kernel norm 0.5 -----
{
  ...
  ##Simulate a stationary FAR(1) process according to a specific kernel
  X.sample_sps <- far_1_1D_no_wn(kernel_id = id_kernel, noise_id = id_noise, n = n, t.grid = t.grid, a = a, burnin = burnin, pert_smooth=C, pert_add=alpha_pert)
}
...
string_message = "
                  PPC prediction data Gaussian Kernel, norm 0.5, no wn "
for (b in 1:N) {
  ...
  prediction_PPC_gau_no_wn_mw[[b]] = PPC_predictor_mw
  err_PPC_gau_no_wn_en_mw[b] = sqrt(MLmetrics::MSE(PPC_predictor_mw$`One-step ahead prediction`,valid_set)) 
  err_PPC_gau_no_wn_rn_mw[b] = MLmetrics::MAE(PPC_predictor_mw$`One-step ahead prediction`,valid_set)
...
if(save_res){
  ...
  res_PPC_gau_no_wn_mw = list(Prediction = prediction_PPC_gau_no_wn_mw, En = err_PPC_gau_no_wn_en_mw, Rn = err_PPC_gau_no_wn_rn_mw)
  file_saving = paste0(path_stor_res_mw,file_saving_PPC_gau_no_wn_mw)
  save(res_PPC_gau_no_wn_mw, file = file_saving)
...
string_message = "
                  PPC prediction data Identity Kernel, norm 0.5, no wn "
for (b in 1:N) {
  ...
  prediction_PPC_id_no_wn_mw[[b]] = PPC_predictor_mw
  err_PPC_id_no_wn_en_mw[b] = sqrt(MLmetrics::MSE(PPC_predictor_mw$`One-step ahead prediction`,valid_set)) 
  err_PPC_id_no_wn_rn_mw[b] = MLmetrics::MAE(PPC_predictor_mw$`One-step ahead prediction`,valid_set)
...
if(save_res){
  ...
  res_PPC_id_no_wn_mw = list(Prediction = prediction_PPC_id_no_wn_mw, En = err_PPC_id_no_wn_en_mw, Rn = err_PPC_id_no_wn_rn_mw)
  file_saving = paste0(path_stor_res_mw,file_saving_PPC_id_no_wn_mw)
  save(res_PPC_id_no_wn_mw, file = file_saving)
...
string_message = "
                  PPC prediction data Sloping Plane t Kernel, norm 0.5, no wn "
for (b in 1:N) {
  ...
  prediction_PPC_spt_no_wn_mw[[b]] = PPC_predictor_mw
  err_PPC_spt_no_wn_en_mw[b] = sqrt(MLmetrics::MSE(PPC_predictor_mw$`One-step ahead prediction`,valid_set)) 
  err_PPC_spt_no_wn_rn_mw[b] = MLmetrics::MAE(PPC_predictor_mw$`One-step ahead prediction`,valid_set)
...
if(save_res){
  ...
  res_PPC_spt_no_wn_mw = list(Prediction = prediction_PPC_spt_no_wn_mw, En = err_PPC_spt_no_wn_en_mw, Rn = err_PPC_spt_no_wn_rn_mw)
  file_saving = paste0(path_stor_res_mw,file_saving_PPC_spt_no_wn_mw)
  save(res_PPC_spt_no_wn_mw, file = file_saving)
...
for (b in 1:N) {
  ...
  prediction_PPC_sps_no_wn_mw[[b]] = PPC_predictor_mw
  err_PPC_sps_no_wn_en_mw[b] = sqrt(MLmetrics::MSE(PPC_predictor_mw$`One-step ahead prediction`,valid_set)) 
  err_PPC_sps_no_wn_rn_mw[b] = MLmetrics::MAE(PPC_predictor_mw$`One-step ahead prediction`,valid_set)
...
if(save_res){
  ...
  res_PPC_sps_no_wn_mw = list(Prediction = prediction_PPC_sps_no_wn_mw, En = err_PPC_sps_no_wn_en_mw, Rn = err_PPC_sps_no_wn_rn_mw)
  file_saving = paste0(path_stor_res_mw,file_saving_PPC_sps_no_wn_mw)
  save(res_PPC_sps_no_wn_mw, file = file_saving)
