...
# ----- storing results -----
prediction_NP_offer  <- lapply((first_prediction):tot_time_instants-1,function(x) NULL)
prediction_NP_demand <- lapply((first_prediction):tot_time_instants-1,function(x) NULL)
total_predictions     <- length(prediction_NP_offer)
...
for (i in 1:total_predictions) {
  ...
  prediction_NP_offer[[i]] = list(Prediction = NP_predictor )
...
if(save_res){
  file_pred_offers = paste0(path_stor_res,"/prediction_NP_offer.Rdata")
  save(prediction_NP_offer, file = file_pred_offers)
}
...
string_message = "
                  NP prediction of demands price "
...
for (i in 1:total_predictions) {
  ...
  prediction_NP_demand[[i]] = list(Prediction = NP_predictor )
...
if(save_res){
  file_pred_demands = paste0(path_stor_res,"/prediction_NP_demand.Rdata")
  save(prediction_NP_demand, file = file_pred_demands)
}
