...
# ----- storing results -----
prediction_NP_offer_log  <- lapply((first_prediction):tot_time_instants-1,function(x) NULL)
prediction_NP_demand_log <- lapply((first_prediction):tot_time_instants-1,function(x) NULL)
total_predictions        <- length(prediction_NP_offer_log)
...
string_message = "
                  NP prediction of log offers price "

for (i in 1:total_predictions) {
  
  NP_predictor = offers_dataset[,i-2+first_prediction]
  
  
  prediction_NP_offer_log[[i]] = list(Prediction = NP_predictor )
...
if(save_res){
  file_pred_offers = paste0(path_stor_res,"/prediction_NP_offer_log.Rdata")
  save(prediction_NP_offer_log, file = file_pred_offers)
}
...
string_message = "
                  NP prediction of log demands price "

for (i in 1:total_predictions) {
  
  NP_predictor = demands_dataset[,i-2+first_prediction]
  
  
  prediction_NP_demand_log[[i]] = list(Prediction = NP_predictor )
...
if(save_res){
  file_pred_demands = paste0(path_stor_res,"/prediction_NP_demand_log.Rdata")
  save(prediction_NP_demand_log, file = file_pred_demands)
}