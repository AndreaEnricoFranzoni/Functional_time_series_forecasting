...
prediction_CC_offer  <- lapply((first_prediction):tot_time_instants-1,function(x) NULL)
prediction_CC_demand <- lapply((first_prediction):tot_time_instants-1,function(x) NULL)
total_predictions     <- length(prediction_CC_offer)
...
for (h in 1:number_of_predictions){
  ...
  prediction_CC_offer[[h]]  = point_prediction$predicted_fun_n_plus_8[[1]]$predicted_p_cg_v
  prediction_CC_demand[[h]] = point_prediction$predicted_fun_n_plus_8[[1]]$predicted_p_cg_a
...
#save results
if(save_res){
  file_pred_offers = paste0(path_stor_res,"/prediction_CC_offer.Rdata")
  save(prediction_CC_offer, file = file_pred_offers)
  
  file_pred_demands = paste0(path_stor_res,"/prediction_CC_demand.Rdata")
  save(prediction_CC_demand, file = file_pred_demands)
