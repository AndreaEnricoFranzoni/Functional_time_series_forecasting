...
#############################################################
#### Computing MP log-prediction as indicated in the readme  ####
#############################################################
...
# ----- storing results -----
prediction_MP_offer_log  <- lapply((first_prediction):tot_time_instants-1,function(x) NULL)
prediction_MP_demand_log <- lapply((first_prediction):tot_time_instants-1,function(x) NULL)
total_predictions        <- length(prediction_MP_offer_log)
...
string_message = "
                  MP prediction of log offers price "
...
for (i in 1:total_predictions) {
  ...
  prediction_MP_offer_log[[i]] = list(Prediction = MP_predictor )
...
#save results
if(save_res){
  file_pred_offers = paste0(path_stor_res,"/prediction_MP_offer_log.Rdata")
  save(prediction_MP_offer_log, file = file_pred_offers)
}
...
string_message = "
                  MP prediction of log demands price "
...
for (i in 1:total_predictions) {
  ...
  prediction_MP_demand_log[[i]] = list(Prediction = MP_predictor )
...
# save results
if(save_res){
  file_pred_demands = paste0(path_stor_res,"/prediction_MP_demand_log.Rdata")
  save(prediction_MP_demand_log, file = file_pred_demands)
}
