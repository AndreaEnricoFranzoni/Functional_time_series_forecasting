...
CP_point_prediction_2D_FAR_EK = function(data_y,
                                         indexes_not_NA=NULL,
                                         l, 
                                         b, 
                                         seed_split=FALSE,
                                         ...
                                         EK_improved=FALSE)
{
  ...
  for (i in training){
    predicted_fun_matrix[i,] = as.numeric(efpc_eval %*% (PSI %*% SCORES[i-1,]))
  } 
  rm(i)
  ...
  for (i in obs_calibration_included){
    predicted_fun_matrix[i,]= efpc_eval %*% (PSI %*% SCORES[i-1,])
  } 
